...
namespace WebTimeSheetManagement.Concrete
{
    public class TimeSheetExportConcrete : ITimeSheetExport
    {
        ...
        public DataSet GetTimeSheetMasterIDTimeSheet(DateTime? FromDate, DateTime? ToDate, int UserID, int AdminID)
        {
            try
            {
                using (SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["TimesheetDBEntities"].ToString()))
        ...
        public DataSet GetMonthlyTimeSheetMasterIDTimeSheet(DateTime? FromDate, DateTime? ToDate, int UserID, int AdminID)
        {
            try
            {
                using (SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["TimesheetDBEntities"].ToString()))
                {
                    DataSet ds = new DataSet();
                    SqlCommand cmd = new SqlCommand("Usp_GetTimeSheetbyFromDateandToDateTimeSheetMonthy", con);
                    cmd.CommandType = System.Data.CommandType.StoredProcedure;
                    cmd.Parameters.AddWithValue("@FromDate", FromDate);
                    cmd.Parameters.AddWithValue("@ToDate", ToDate);
                    cmd.Parameters.AddWithValue("@UserID",UserID);
